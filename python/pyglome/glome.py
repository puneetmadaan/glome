...
class AutoGlome():
    ...
    This class is initialized by providing your peer's public key and
    optionally your private key. If a private key is not provided, one is
    automatically genera ted making use of `generate_keys`. On initialization,
    ...
    def __init__(self,
        ...
        """ Adds counter managing to the Glome Class.

        Performs the handshake and generates keys and counters.
        Args:
           peer_key: Your peer's public key.
           my_private_key: Yours private key.
           min_peer_tag_len: Desired length (in bytes) for the tag.
             Must be an integer in range 8-32.
           skippable_range: Number of messages that can be missed.
        Raises:
           ValueError: Raised whenever min_peer_tag_len is not in range 8-32 or
    ...
    def sending_counter(self) -> int:
        """Counter that is used to create tag sending messages.

        It is incremented each time a new tag is generated. It is always
        one byte long. As a consequence when the counter gets past 255 it
        restarts at 0.
    ...
    def receiving_counter(self) -> int:
        """Counter that is used to check tag from receiving  messages.

        It is incremented each time a new tag is generated. It is
        always one byte long, therefore when the counter gets past 255
        it restarts at 0.
    ...
    def check(self, tag: bytes, msg: bytes):
        ...
        Checks if a tag matches some provided message and internal
        receiving counter. The method generates the matching tag
        following GLOME protocol specification in the context of a
        communication from the users' peers to the users and then
        is compared with the tag provided. If the tag is not validated 
        the counter remains intact.
